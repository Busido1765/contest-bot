...
## Перед запуском: чек комплектности

Перед любыми `docker build` проверьте, что в рабочем дереве есть ожидаемые каталоги исходников:

```bash
test -d src/internal && echo "src/internal ok" || echo "src/internal missing"
test -d src/miniapp && echo "src/miniapp ok" || echo "src/miniapp missing"
```

Если один из путей отсутствует, сначала синхронизируйте исходники (другая ветка, сабмодуль или приватный репозиторий с кодом).
