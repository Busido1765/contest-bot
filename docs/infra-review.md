...
## 0.1) Как понять, что блокер по `src/` снят

Блокер считается закрытым, когда одновременно выполняются условия:

```bash
test -d src/internal && echo ok || echo fail
test -d src/miniapp && echo ok || echo fail
```

После этого можно переходить к обычному quickstart (`docker build`/`docker compose up`).
